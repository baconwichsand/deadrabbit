...
def get_historical_prices(symbol, start_date, end_date):
    ...
    for day in daily_data[1:]:
        day_data = day.split(',')
        # new_date = float(day_data[0].replace('-', ''))
        # output.append([new_date, float(day_data[1]), float(day_data[2]), float(day_data[3]), float(day_data[4]), float(day_data[5]), float(day_data[6])])
        date = day_data[0]
        hist_dict[date] = \
            {keys[1]: day_data[1],
             keys[2]: day_data[2],
